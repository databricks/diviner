...
from diviner.config.grouped_prophet.prophet_config import get_scoring_metrics, _get_extract_params
...
def test_base_cv_metrics_extract():
    ...
    metric_result_positive_uncertainty = prophet_config_utils._reconcile_metrics(base, init_prophet.uncertainty_samples)

    assert "coverage" in metric_result_positive_uncertainty

    setattr(init_prophet, "uncertainty_samples", 0)

    metric_result_no_uncertainty = prophet_config_utils._reconcile_metrics(base, init_prophet.uncertainty_samples)
