...
This document is a hands-on manual for doing issue and pull request triage for `Diviner issues
on GitHub <https://github.com/databricks/diviner/issues>`_ .
...
- | ``priority/awaiting-more-evidence``: Lowest priority. Possibly useful, but not yet enough
  ...
  | If we think it doesn’t fit in Diviner, we should just say that and why.

Label relevant areas
####################

Assign one more labels for relevant component areas. As a principle, we aim to have the minimal set of labels
