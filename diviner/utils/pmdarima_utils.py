...
class PmdarimaUtils:
    def __init__(self, df, group_key_columns, y_col, datetime_col):
        ...
        `decompose_groups`: trend decomposition into 'trend', 'seasonal' and 'random' elements
                            that collectively make up the underlying series data.
        `calculate_ndiffs`: optimal selection of the `d` (differencing) term for ARIMA to convert
                            a series to one that is stationary. Specifying this as a constant to
                            AutoARIMA can drastically reduce training time.
        `calculate_nsdiffs`: optimal selection of the `D` (seasonal differencing) term for
                             SARIMAX to convert a seasonally-influenced series to a staionary one.
                             Providing this as a constant to the AutoARIMA args `D=<value>` can
                             reduce training time, eliminating recursive loops during optimization.
        ...
        :param df: The DataFrame input consisting of a `y_col`, grouping columns, and a date (or
                   datetime) column to be analyzed.
    ...
    def calculate_acf(
        ...
        :param unbiased: Boolean flag that sets the autocovariance denominator to `n-k` if True
                         and `n` if False.
                         Default: False
        ...
        :param missing: handling of NaN values in the series data. Available options:
                        ...
                        `drop`: NaN values are removed from the series and adjacent values to NaN's
                                are treated as contiguous (which may invalidate the results in
                                certain situations).
                        Default: "none"
    ...
    def generate_diff_inversion(group_diff_data, lag=1, differences=1, recenter=False):
        ...
        :param recenter: If True and "series_start" exists in `group_diff_data` dict, will restore
                         the original series range for each group based on the series start value
                         calculated through the `generate_diff()` method.
